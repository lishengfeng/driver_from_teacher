...
void ft5x06_report(struct ft5x06_ts_info *ts)
{
        ...
        if(tp){
                for(i = 0; i < tp; i++){
                        x = ((ts->buf[3 + i * 6] & 0xf) << 8) | ts->buf[4 + i * 6];
                        y = ((ts->buf[5 + i * 6] & 0xf) << 8) | ts->buf[6 + i * 6];

                        input_report_abs(ts->idev, ABS_MT_POSITION_X, x);
                        input_report_abs(ts->idev, ABS_MT_POSITION_Y, y);
                        /*必须报告挤压事件*/ 
                        input_report_abs(ts->idev, ABS_MT_PRESSURE, 1);
                        /*触点的序号*/
                        input_report_abs(ts->idev, ABS_MT_TRACKING_ID, i);
...
void do_ft5x06_work(struct work_struct *work)
{
        ...
        ret = ft5x06_read(ts->ic, 0, ts->buf, POINT_NUM);
        if(ret < 0)
                return;
...
#ifdef MUTIL_POINT
        set_bit(ABS_MT_POSITION_X, ts->idev->absbit);	
        set_bit(ABS_MT_POSITION_Y, ts->idev->absbit);	
        set_bit(ABS_MT_TRACKING_ID, ts->idev->absbit);
        set_bit(ABS_MT_PRESSURE, ts->idev->absbit);

        input_set_abs_params(ts->idev, ABS_MT_PRESSURE, 0, 200, 0, 0);
        input_set_abs_params(ts->idev, ABS_MT_POSITION_X, 0, 800, 0, 0);
        input_set_abs_params(ts->idev, ABS_MT_POSITION_Y, 0, 480, 0, 0);
        input_set_abs_params(ts->idev, ABS_MT_TRACKING_ID, 0, MUTIL_POINT, 0, 0);
#else
